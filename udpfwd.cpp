...
int run(u_short port) {
    ...
    for (;;) {
        ...
        if (fds[0].revents) {
            ...
            std::cout << "Got " << rsize << " bytes from client: " << ntohs(from.sin_port) << std::endl;
            char hostname[256];
        ...
        for (int i = 1; i <= client_count; i++) {
            if (fds[i].revents) {
                ...
                std::cout << "Got " << rsize << " bytes from server" << std::endl;
                auto client = gClients[sock];
                ssize_t sent;
                sent = ::sendto(listen_sock, buf, rsize, 0,
